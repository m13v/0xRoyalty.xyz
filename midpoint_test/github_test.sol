 ...
 *
 * This is a contract generated at 2022-11-19 01:08:48 for making requests to and receiving responses from midpoint 477. 
...
contract SampleContract {
    ...
    event RequestMade(uint256 requestId, string repository);
    event ResponseReceived(uint256 requestId, string[] name, string[] login, int32[] commits);
    
    address constant startpointAddress = 0x9BEa2A4C2d84334287D60D6c36Ab45CB453821eB;
    address constant whitelistedCallbackAddress = 0x2ae21EE1eFEf4dceED684Ad74A9e342D5428E6AE;
    
    // Midpoint ID
    uint64 constant midpointID = 477;
     ...
     * 
     * Note that this is a public function and will allow any address or contract to call midpoint 477.
    ...
    function callMidpoint(string memory repository) public {
        
        // Argument String
        bytes memory args = abi.encodePacked(repository, bytes1(0x00));
        
        // Call Your Midpoint
        uint256 requestId = IMidpoint(startpointAddress).callMidpoint(midpointID, args);

        // For Demonstration Purposes Only
        emit RequestMade(requestId, repository);
    }
   ...
   function callback(uint256 _requestId, uint64 _midpointId, string[] memory name, string[] memory login, int32[] memory commits) public {
       ...
       // For Demonstration Purposes Only
       emit ResponseReceived(_requestId, name, login, commits);
   }
}
